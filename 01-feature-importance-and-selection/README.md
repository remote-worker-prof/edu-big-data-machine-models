...
- `study-notes/` — отдельные заметки для углубленного самостоятельного изучения методов/подходов.
...
2. Если объяснение слишком объемное, вынесите его в `study-notes/*.md`.
3. Добавьте ссылку на заметку обратно в ноутбук и затем в отчет.
4. Убедитесь, что ссылка сопровождается кратким комментарием “зачем эта заметка”.
...
5. Выполнить обязательные самостоятельные задания с intentional-stop.
6. Проверить, что все обязательные CSV-артефакты сохранены в `outputs/`.
7. Заполнить отчет по `report-template.md`.
