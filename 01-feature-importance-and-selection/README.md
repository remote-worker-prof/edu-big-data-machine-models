# Лабораторная работа 01: Feature Importance and Selection

## О чем эта работа
Эта лабораторная знакомит с практическим отбором признаков без углубления в тяжелые математические доказательства.
Вы последовательно сравните несколько подходов к оценке значимости признаков и проверите, как отбор влияет на качество моделей.

## Формат
- 3 обязательных Jupyter-ноутбука.
- 2 прикладных бинарных классификационных датасета:
  - медицина: прогноз сердечно-сосудистого риска;
  - финансы: прогноз кредитного риска.
- Локальный запуск на CPU.
- Данные в CSV лежат внутри репозитория.

## Вход Для Новичка (Перед Практикой)
- Что уже нужно знать:
  - что такое бинарная классификация;
  - базовые метрики `accuracy`, `f1`, `roc_auc`;
  - как запускать Jupyter notebook.
- Что можно пропустить на первом проходе:
  - детали формул методов отбора;
  - углубленные сравнения beyond базового маршрута.
- Какие артефакты должны быть на диске перед стартом:
  - в ЛР01 upstream-артефакты не требуются;
  - достаточно исходных CSV из `data/`.

## Структура папки
- `data/` — исходные датасеты и краткое описание колонок.
- `notebooks/` — шаблоны заданий с TODO + обязательные самостоятельные блоки (без образцов решений).
- `solutions/` — готовые решения только для базового маршрута ноутбуков.
- `study-notes/` — отдельные заметки для углубленного самостоятельного изучения методов/подходов и обязательный `glossary.md`.
- `outputs/` — промежуточные и итоговые таблицы (`feature_ranking`, `model_results`).
- `report-template.md` — шаблон итогового отчета по лабораторной работе.
- `requirements.txt` — зависимости.

## Ноутбуки и порядок прохождения
1. `notebooks/01_filter_methods_todo.ipynb` (90 минут)
   - базовый pipeline;
   - `VarianceThreshold`, корреляционный анализ, `mutual_info_classif`, `f_classif`;
   - формирование `feature_ranking` и shortlist.
2. `notebooks/02_wrapper_embedded_todo.ipynb` (90 минут)
   - `RFE`, `SequentialFeatureSelector`, L1-регуляризация;
   - важности деревьев и permutation importance;
   - формирование 2-3 кандидатных feature set.
3. `notebooks/03_model_comparison_todo.ipynb` (60 минут)
   - сравнение `LogisticRegression`, `RandomForest`, `LinearSVC`;
   - опциональный блок `MLPClassifier`;
   - формирование `model_results` и практических выводов.

### Workflow: base + mandatory independent
- **Базовый маршрут**: закрывается по `solutions/*_solution.ipynb`.
- **Обязательные самостоятельные блоки**: выполняются только в `notebooks/*_todo.ipynb`.
- В `solutions` нет ответов на самостоятельные блоки намеренно.
- По ходу выполнения ноутбуков обязательно фиксировать, **что изучено по методам**:
  narrative + сравнения + источники (в ноутбуке и/или в `study-notes/*.md`).

## Тайминг
- Ядро: 4 академических часа (90 + 90 + 60 минут).
- Расширенный трек: 1-2 дня (опциональные эксперименты и более глубокая интерпретация).

## Что сдавать
- Заполненные ноутбуки (выполненные ячейки и комментарии с выводами).
- Все дополнительные CSV-артефакты из обязательных самостоятельных блоков.
- Заполненные narrative-блоки по самостоятельному изучению методов
  (ссылки на источники и при необходимости на `study-notes/*.md`).
- Обновленный глоссарий `study-notes/glossary.md`:
  минимум 3 новых термина на каждый ноутбук с простыми объяснениями и источниками.
- Краткий отчет по шаблону `report-template.md`.

## Формат промежуточных таблиц
`feature_ranking`:
- `dataset` — название датасета;
- `method` — метод отбора/оценки;
- `feature` — признак;
- `score` — числовой скор;
- `rank` — ранг внутри `dataset + method`.

`model_results`:
- `dataset` — название датасета;
- `feature_set` — набор признаков (`full`, `set_A`, ...);
- `model` — название модели;
- `metric` — метрика (`accuracy`, `f1`, `roc_auc`);
- `value` — значение метрики;
- `fit_time_sec` — время обучения модели в секундах.

## Обязательные самостоятельные блоки (только в notebooks)
В каждом `*_todo.ipynb` добавлен раздел `Обязательные самостоятельные задания (без образца в solutions)`.

Важно:
- эти задания **обязательны** к выполнению;
- в шаблонах стоят intentional-stop ячейки с `NotImplementedError`;
- пока шаблон не заменен своим кодом, ноутбук останавливается в этом разделе.
- Дополнительно по ходу выполнения в релевантных местах обязательны блоки
  `Самостоятельное изучение по ходу работы` (что изучено, сравнение, источники).
- Если разбор метода слишком объемный, он выносится в `study-notes/*.md`
  с обязательной ссылкой из ноутбука и отчета.

Дополнительные обязательные артефакты:
- `outputs/filter_stability_grid.csv`
- `outputs/method_agreement_long.csv`
- `outputs/selection_stability.csv`
- `outputs/threshold_tuning_results.csv`
- `outputs/cv_stability_results.csv`
- `outputs/error_by_segment.csv`

### Краткий workflow для `study-notes/`
1. В релевантном месте ноутбука заполните narrative-блок:
   что изучено, сравнение, источники.
2. По ходу выполнения пополняйте `study-notes/glossary.md` незнакомыми терминами
   (простое объяснение, где встретился термин, источник).
3. Если объяснение слишком объемное, вынесите его в `study-notes/*.md`.
4. Добавьте ссылку на заметку обратно в ноутбук и затем в отчет.
5. Убедитесь, что ссылка сопровождается кратким комментарием “зачем эта заметка”.

## Запуск
Команды выполняются из папки `01-feature-importance-and-selection`.

```bash
python3 -m venv .venv
.venv/bin/python -m pip install --upgrade pip
.venv/bin/python -m pip install -r requirements.txt
.venv/bin/python -m ipykernel install --user --name feature-lab --display-name "Python (.venv) Feature Lab"
.venv/bin/python -m jupyter notebook
```

Опционально можно использовать `source .venv/bin/activate` и запускать команды через `python`.

## Проверка Для Преподавателя/Разработчика
Из папки ЛР 01:

```bash
.venv/bin/python scripts/verify_lab01.py
```

Скрипт исполняет `solution`-ноутбуки и проверяет структуру `todo/solution`, а также контракты артефактов в `outputs/`.

## Предсдача Для Студента
```bash
.venv/bin/python ../scripts/check_submission.py --lab 01
```

## Что Делать, Если Не Запускается
1. Ошибка `ModuleNotFoundError`:
   - выполните `.venv/bin/python -m pip install -r requirements.txt`.
2. Не запускается `jupyter`:
   - проверьте `.venv/bin/python -m jupyter --version`;
   - при ошибке переустановите зависимости команды выше.
3. Скрипты проверки не видят файлы:
   - убедитесь, что вы запускаете команды из папки ЛР01;
   - используйте `pwd` и сверяйте путь.
4. Для полной диагностики среды из корня репозитория:
   - `.venv/bin/python scripts/doctor_env.py`.

## Рекомендуемый маршрут выполнения
1. Пройти базовые шаги `01_filter_methods_todo.ipynb` и заполнить блоки `Самостоятельное изучение по ходу работы`.
2. Пройти базовые шаги `02_wrapper_embedded_todo.ipynb` и заполнить блоки `Самостоятельное изучение по ходу работы`.
3. Пройти базовые шаги `03_model_comparison_todo.ipynb` и заполнить блоки `Самостоятельное изучение по ходу работы`.
4. При сложных методах оформить отдельные заметки в `study-notes/*.md` и добавить ссылки из ноутбуков.
5. По ходу каждого ноутбука обновлять `study-notes/glossary.md` (минимум 3 новых термина).
6. Выполнить обязательные самостоятельные задания с intentional-stop.
7. Проверить, что все обязательные CSV-артефакты сохранены в `outputs/`.
8. Заполнить отчет по `report-template.md`.

## Траектории Прохождения
- Легкий трек (минимально обязательный маршрут):
  - пройти три `todo`-ноутбука и закрыть все `TODO(обязательно)`;
  - сформировать обязательные CSV-артефакты;
  - заполнить отчет и базовый глоссарий.
- Углубленный трек:
  - повторить эксперименты с другими гиперпараметрами отбора (число признаков, пороги);
  - добавить кросс-валидацию и доверительные интервалы метрик;
  - активировать опциональный блок `MLPClassifier` и сравнить с классическими моделями.
